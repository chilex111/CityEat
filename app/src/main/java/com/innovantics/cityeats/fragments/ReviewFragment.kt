...
class ReviewFragment : Fragment() {
    ...
    override fun onActivityCreated(savedInstanceState: Bundle?) {
        ...
        txtTitle.text = getString(R.string.reviews)
        setUpRecyclerView()
    }

    private fun setUpRecyclerView() {
        val reviewAdapter = ReviewAdapter(activity!!,1)
        dialog_recycler.layoutManager = LinearLayoutManager(activity)
        dialog_recycler.adapter = reviewAdapter
        dialog_recycler.setHasFixedSize(true)
        dialog_recycler.requestFocus()
    }
