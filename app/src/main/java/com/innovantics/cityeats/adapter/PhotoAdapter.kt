...
class PhotoAdapter(/*private var accountList: List<String>,*/ private var context: Context)
    : RecyclerView.Adapter<PhotoAdapter.AcctViewHolder>() {

    companion object {
       var stringListener: GenericListener<String>?= null
    }
    class AcctViewHolder(inflate: View): RecyclerView.ViewHolder(inflate) {
        private var photos: ImageView? = null
        init {
            photos = itemView.findViewById(R.id.imgPhoto)
        }
    }

    override fun onCreateViewHolder(paramViewGroup: ViewGroup, paramInt: Int): AcctViewHolder {
        val inflater = context.getSystemService(Context.LAYOUT_INFLATER_SERVICE) as LayoutInflater
        val v = inflater.inflate(R.layout.custom_photo, paramViewGroup, false)
